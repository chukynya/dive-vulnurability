"""Build dive-6.ipynb: GraphCodeBERT (frozen) over function-level source units.
...
# ---------------------------------------------------------------------------
md("""# DIVE-6 — GraphCodeBERT (frozen) over function-level source units
...
- `RESUME_FROM = "/kaggle/working/last_state.pt"` -> continue from saved state.
  ...
  calibrators are restored. The stage-1 embedding cache (`dive6_units.npz`) is
...
Mirrors stdout to `dive6_train.log`. Survives Kaggle "Save Version" and is
...
2. Frozen GraphCodeBERT mean-pools each unit ( <=256 tokens ) into a 768-d
   vector — cached once to `dive6_units.npz`.
...
- `cache/dive6_units.npz` — frozen unit embeddings ( reused on resume ).
- `best_model.pt` — EMA weights + calibrators + frozen thresholds.
- `last_state.pt` — full resumable state ( `RESUME_FROM` ).
- `metrics.json`, `per_class.csv`, `history.csv`, `dive6_train.log`.
...
out = Path(__file__).parent / "dive-6.ipynb"
