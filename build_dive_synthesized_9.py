"""Build dive-synthesized-9.ipynb: dive-synthesized-7 (frozen GraphCodeBERT source) + stronger aggregator regularization, on the synthetic-augmented 80/10/10 dive-synthesized dataset.
...
# ---------------------------------------------------------------------------
md("""# dive-synthesized-9 — dive-synthesized-7 + stronger regularization

**This run.** Identical to dive-synthesized-7 (frozen GraphCodeBERT + set-Transformer on the
synthetic-augmented 80/10/10 source split, cached two-stage). The ONE change: **stronger
regularization** on the aggregator — DROPOUT 0.20→0.30, DROP_PATH 0.10→0.20,
WEIGHT_DECAY 0.05→0.10 — to close dive-7's train→test gap (train ~0.90 vs test 0.79).
Baseline to beat: dive-synthesized-7 test macro-F1 **0.7906**.
...
- `RESUME_FROM = "/kaggle/working/last_state.pt"` -> continue from saved state.
  ...
  calibrators are restored. The stage-1 embedding cache (`dive_synth9_units.npz`) is
...
Mirrors stdout to `dive_synth9_train.log`. Survives Kaggle "Save Version" and is
...
2. Frozen GraphCodeBERT mean-pools each unit ( <=256 tokens ) into a 768-d
   vector — cached once to `dive_synth9_units.npz`.
...
- `cache/dive_synth9_units.npz` — frozen unit embeddings ( reused on resume ).
- `best_model.pt` — EMA weights + calibrators + frozen thresholds.
- `last_state.pt` — full resumable state ( `RESUME_FROM` ).
- `metrics.json`, `per_class.csv`, `history.csv`, `dive_synth9_train.log`.
...
out = Path(__file__).parent / "dive-synthesized-9.ipynb"
