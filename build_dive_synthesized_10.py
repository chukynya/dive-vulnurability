"""Build dive-synthesized-10.ipynb: dive-synthesized-9 + diversified BR/FR synthetic data + enhanced loss/F1 graphs.
...
# ---------------------------------------------------------------------------
md("""# dive-synthesized-10 — dive-synthesized-9 + diversified BR/FR synthetic

**This run.** Identical to dive-synthesized-9 (frozen GraphCodeBERT + set-Transformer on the
synthetic-augmented 80/10/10 source split, cached two-stage). The ONE change: the
**dive-synthesized dataset is updated** — the old templated Bad Randomness / Front Running
synthetic contracts are replaced with **diversified LLM-generated BR/FR contracts** that span
16+ distinct patterns per class (vs. 1-2 fill-in-the-blank templates before).
Baseline to beat: dive-synthesized-9 test macro-F1 **0.7854**.

**Why this matters.** In dive-9, BR and FR were the two weakest classes (F1 0.737 and 0.645).
The old synthetic data for those classes was templated — every injected contract looked nearly
identical. The new diverse BR/FR synthetic should give the aggregator richer training signal
for those patterns, targeting precision on rare-class contracts.

**New additions in this notebook.** Beyond the dataset change:
- `val_loss` tracked per epoch (alongside `train_loss`) for clean loss-curve overfit diagnosis.
- `test_loss` reported at final eval.
- 4 diagnostic graphs: (1) loss curves, (2) macro-F1 curves, (3) per-class F1 bars train/val/test,
  (4) per-class overfit gap chart with color-coded severity.
- Per-class val F1 logged whenever a new best is found.
- Cleaner per-epoch log format (3 lines instead of one wall of text).

## What stays identical to dive-9
...
- DROPOUT=0.30, DROP_PATH=0.20, WEIGHT_DECAY=0.10 (stronger regularization from dive-9).
...
- Atomic full-state checkpointing every epoch + every 30 min, tee logger, history.csv.

## Reading the result

1. **Gate logs (cells 4-8):** source coverage, units/contract, tokens/unit truncation, stage-1 timing.
...
- `RESUME_FROM = "/kaggle/working/last_state.pt"` -> continue from saved state.
  ...
  calibrators are restored. The stage-1 embedding cache (`dive_synth10_units.npz`) is
...
Mirrors stdout to `dive_synth10_train.log`. Survives Kaggle "Save Version" and is
...
code("""try:
    ...
    ax.set_title("dive-synthesized-10 — loss curves")
    ...
    ax.set_title("dive-synthesized-10 — macro-F1 curves")
    ax.legend(fontsize=9); ax.grid(alpha=0.3); fig.tight_layout()
    ...
    ax.set_title("dive-synthesized-10 — per-class F1: train vs val vs test")
    ...
    ax.set_title("dive-synthesized-10 — per-class overfit gap (train − test)")
    ax.legend(handles=[
...
2. Frozen GraphCodeBERT mean-pools each unit ( <=256 tokens ) into a 768-d
   vector — cached once to `dive_synth10_units.npz`.
...
- `cache/dive_synth10_units.npz` — frozen unit embeddings ( reused on resume ).
- `best_model.pt` — EMA weights + calibrators + frozen thresholds.
- `last_state.pt` — full resumable state ( `RESUME_FROM` ).
- `metrics.json`, `per_class.csv`, `history.csv`, `dive_synth10_train.log`.
...
out = Path(__file__).parent / "dive-synthesized-10.ipynb"
