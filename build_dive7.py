"""Build dive-7.ipynb: faster GraphCodeBERT inference via fewer, sharper units.

dive-6 (frozen GraphCodeBERT over function-level source units) reached test
macro-F1 0.768 — good — but end-to-end inference is ~125 ms p50 on a T4,
dominated by encoding ~28 units/contract through the 125M-param model. dive-7
keeps the *entire* dive-6 pipeline and changes only the encoded input budget:

  - MAX_UNITS 32 -> 12  (the risk-ranked cap already keeps contract headers +
    the most vulnerable functions first; dive-6 logs showed 69% of contracts
    hit the 32 cap, and most dropped units are boilerplate getters).
  - UNIT_TOKENS 256 -> 128  (median unit is 64 tokens, so 128 covers most with
    far less padding waste; dive-6 truncated 11.5% at 256 regardless).

Net: ~5x fewer encoded token-positions/contract -> expected ~2-4x faster
inference. Hypothesis: F1 holds (or improves slightly, less boilerplate noise).
The single variable is the input budget; everything else is identical to dive-6,
so the F1 / latency deltas are clean.

Reused verbatim from dive-6: the Solidity splitter + risk-rank, frozen
graphcodebert-base mean-pooled unit embeddings (cached), the set-Transformer
aggregator, seed-42 split, sampler, AsymmetricLoss + aux BCE, EMA, isotonic
calibration -> threshold tuning, checkpointing, and the end-to-end inference
benchmark cell (compare its p50 ms against dive-6's 124.73 ms).
"""
import json
...
# ---------------------------------------------------------------------------
md("""# DIVE-7 — Faster GraphCodeBERT: fewer, sharper units

**Why this notebook.** dive-6 (frozen GraphCodeBERT over function-level source
units) reached **test macro-F1 0.768** — a clear gain over dive-5's 0.747,
concentrated in the rare classes (Bad Randomness 0.65->0.72, Front Running
0.57->0.62, DoS 0.63->0.65). But end-to-end inference is **~125 ms p50** on a
T4, almost entirely the cost of encoding ~28 units/contract through the
125M-param model.

dive-7 keeps the **entire dive-6 pipeline** and changes only the **encoded input
budget**, to cut that latency while holding F1.

## What's new vs dive-6

| Change | Why |
|---|---|
| **MAX_UNITS 32 -> 12** | dive-6 logged 69% of contracts hitting the 32-unit cap; the risk-rank already keeps contract headers + the most vulnerable functions first, so the dropped units are mostly boilerplate getters. Fewer units = fewer encoded sequences. |
| **UNIT_TOKENS 256 -> 128** | Median unit is 64 tokens, so 128 covers most with far less padding waste; dive-6 truncated 11.5% of units at 256 regardless. Halves the per-unit sequence length. |

Combined, **~5x fewer encoded token-positions per contract** -> expected ~2-4x
faster inference. **Hypothesis:** macro-F1 holds or improves slightly (less
boilerplate noise feeding the aggregator).

## What stays identical to dive-6 (the clean-comparison contract)

Everything else: the Solidity function splitter + risk-rank scoring, frozen
graphcodebert-base mean-pooled unit embeddings, the set-Transformer aggregator,
seed-42 split, sqrt-inv-freq sampler, AsymmetricLoss + 0.3*aux BCE on BR/FR,
EMA(0.999), per-class isotonic calibration -> 41-pt threshold tuning, atomic
checkpointing, tee logger, `torch.load(weights_only=False)`.

## Reading the result

1. **`=== INFERENCE BENCHMARK ===` p50_ms** vs dive-6's **124.73 ms** — the speed win.
2. **`test_calibrated_tuned.f1_macro` vs dive-6's 0.768** — did F1 hold?
3. **Per-class F1** — watch whether dropping to 12 units costs the
   multi-vulnerability contracts (classes that need many functions in view).
""")
...
- `RESUME_FROM = "/kaggle/working/last_state.pt"` -> continue from saved state.
  ...
  calibrators are restored. The stage-1 embedding cache (`dive7_units.npz`) is
...
MAX_UNITS    = 12           # dive-7: 32 -> 12 (risk-ranked cap keeps the most vulnerable units)
UNIT_TOKENS  = 128          # dive-7: 256 -> 128 (median unit is 64 tokens; less padding waste)
...
assert LABEL_CSV.exists(), f"Missing {LABEL_CSV}"
assert SOURCE_DIR.exists(), f"Missing source dir {SOURCE_DIR} — upload the .sol files to the Kaggle dataset"
print("Inputs OK:", LABEL_CSV.name, "|", SOURCE_DIR.name, flush=True)
...
Mirrors stdout to `dive7_train.log`. Survives Kaggle "Save Version" and is
...
# ---------------------------------------------------------------------------
md("""## 5 — Load labels + source availability ( verification gate #2 )

Build `df` = label rows that have a `{contractID}.sol` file, sorted by
`contractID`. Assert source coverage >= `MIN_SOURCE_COVERAGE`. The split is the
same methodology as dive-5 but over the source-available universe.
""")

code("""labels_df = pd.read_csv(LABEL_CSV)
log(f"Labels shape: {labels_df.shape}")

have_src = labels_df["contractID"].apply(lambda c: (SOURCE_DIR / f"{int(c)}.sol").exists())
coverage = have_src.mean()
log(f"Source coverage: {coverage:.2%} of {len(labels_df)} label rows have a .sol file")
assert coverage >= MIN_SOURCE_COVERAGE, \\
    f"Only {coverage:.2%} of label rows have source — expected >= {MIN_SOURCE_COVERAGE:.0%}. " \\
    f"Check the Source/ folder is in the Kaggle dataset."

df = labels_df[have_src].sort_values("contractID").reset_index(drop=True)
log(f"Working set: {df.shape}")
log("Positives per class:")
for c in LABEL_COLS:
    log(f"  {c:>26s}  {int(df[c].sum()):>5d}")
""")

# ---------------------------------------------------------------------------
md("""## 6 — Stratified 80/10/10 split

Same seed (42) and proportions as dive-3/4/5.
""")
...
def read_source(cid):
    try:
        return (SOURCE_DIR / f"{int(cid)}.sol").read_text(encoding="utf-8", errors="ignore")
    except Exception:
        return ""
...
**dive-7 = dive-6 with a smaller encoded input budget.** Same pipeline, fewer
and shorter units, to cut inference latency while holding F1.

**The single change.** `MAX_UNITS` 32 -> 12 and `UNIT_TOKENS` 256 -> 128. The
risk-rank keeps the most vulnerable units; ~5x fewer encoded token-positions
per contract.

**Pipeline (unchanged from dive-6).**
1. Split each `.sol` into function/header units ( brace-matching, robust across
   0.4-0.8 ), risk-ranked cap to `MAX_UNITS`.
2. Frozen GraphCodeBERT mean-pools each unit ( <=`UNIT_TOKENS` ) into a 768-d
   vector — cached once to `dive7_units.npz`.
3. A set-Transformer aggregates the unit set and feeds the 8 per-class MLP heads
   + aux BCE on BR/FR, same loss / sampler / EMA / calibration / thresholds.

**Outputs (`/kaggle/working/`).**
- `cache/dive7_units.npz` — frozen unit embeddings ( reused on resume ).
- `best_model.pt`, `last_state.pt`, `metrics.json`, `per_class.csv`,
  `history.csv`, `dive7_train.log`, `inference_bench.json`.

**What to look at.**
1. `=== INFERENCE BENCHMARK ===` p50_ms vs dive-6's 124.73 ms — the speed win.
2. `test_calibrated_tuned.f1_macro` vs dive-6's 0.768 — did F1 hold?
3. Per-class F1 — whether 12 units costs the multi-vulnerability contracts.

**Next lever:** if F1 held here, the path to *higher* F1 ( not just faster ) is
unfreezing the top GraphCodeBERT layers for end-to-end fine-tuning ( dive-8 ).
""")
...
out = Path(__file__).parent / "dive-7.ipynb"
