"""Build dive-synthesized-8.ipynb: dive-6 source pipeline with the TOP GraphCodeBERT layers FINE-TUNED (single-stage, no embedding cache), on the synthetic-augmented 80/10/10 dive-synthesized dataset.
...
# ---------------------------------------------------------------------------
md("""# dive-synthesized-8 — top GraphCodeBERT layers FINE-TUNED (single-stage)

**This run.** Same data as dive-synthesized-7 (synthetic-augmented 80/10/10, source). The
change vs dive-7: the top **UNFREEZE_TOP_K** GraphCodeBERT layers are **trainable**, so the
encoder adapts to Solidity. That breaks the cached two-stage design — GraphCodeBERT now runs
in the training loop every step — so units/tokens/batch are **capped small** (MAX_UNITS=10,
UNIT_TOKENS=128, batch=8) with a **wall-clock guard** that stops at ~7h keeping the best
checkpoint. Discriminative LR (encoder 1e-5, aggregator 3e-4). Baseline to beat:
dive-synthesized-7 test macro-F1 **0.7906**. Caveat: unfreeze + the smaller unit cap change
together, so the delta isn't a clean one-variable comparison.
...
- `RESUME_FROM = "/kaggle/working/last_state.pt"` -> continue from saved state.
  ...
  calibrators are restored. The tokenised-unit cache (`dive_synth8_tokens.npz`) is
  reused if present.
""")
...
MAX_UNITS    = 10           # function/header units per contract (small: end-to-end FT budget)
UNIT_TOKENS  = 128          # GraphCodeBERT token cap per unit (small: FT budget)
...
GCB_BATCH    = 64           # units per tokeniser batch (stage-1 is now tokenise-only)

# -- Fine-tuning (dive-8): unfreeze top GraphCodeBERT layers, single-stage ----
UNFREEZE_TOP_K   = 2        # top encoder layers to fine-tune (the rest stay frozen)
ENC_LR           = 1e-5     # discriminative LR for the unfrozen encoder layers
GRAD_CHECKPOINT  = False    # set True only if you raise the caps and hit OOM
TIME_BUDGET_SECS = int(7.0 * 3600)   # wall-clock guard: stop & keep best before Kaggle's limit
...
# -- Training -----------------------------------------------------------------
BATCH_SIZE   = 8            # contracts/step; each = up to MAX_UNITS unit-forwards through GraphCodeBERT
EPOCHS       = 8
LR           = 3e-4
WEIGHT_DECAY = 0.05
GRAD_CLIP    = 1.0
...
Mirrors stdout to `dive_synth8_train.log`. Survives Kaggle "Save Version" and is
...
gcb = AutoModel.from_pretrained(GCB_MODEL, add_pooling_layer=False).to(device)
# Freeze everything, then unfreeze the top-K encoder layers for fine-tuning.
for p in gcb.parameters():
    p.requires_grad_(False)
enc_layers = gcb.encoder.layer
for layer in enc_layers[-UNFREEZE_TOP_K:]:
    for p in layer.parameters():
        p.requires_grad_(True)
if GRAD_CHECKPOINT:
    try:
        gcb.gradient_checkpointing_enable(gradient_checkpointing_kwargs={"use_reentrant": False})
    except TypeError:
        gcb.gradient_checkpointing_enable()
    gcb.config.use_cache = False

hidden = gcb.config.hidden_size
n_total = sum(p.numel() for p in gcb.parameters())
n_train = sum(p.numel() for p in gcb.parameters() if p.requires_grad)
log(f"Loaded {GCB_MODEL} in {time.time()-t0:.1f}s | hidden={hidden} | params={n_total/1e6:.1f}M | "
    f"trainable top {UNFREEZE_TOP_K}/{len(enc_layers)} layers = {n_train/1e6:.2f}M | grad_ckpt={GRAD_CHECKPOINT}")
...
# ---------------------------------------------------------------------------
md("""## 8 — Stage-1: tokenise units ( no encoding — GraphCodeBERT is trained now )

Because the top encoder layers are fine-tuned, unit embeddings change every step,
so they can't be pre-cached. Instead we pre-**tokenise** every unit once into compact
`(N, MAX_UNITS, UNIT_TOKENS)` int arrays (~a couple hundred MB at these caps); the
trainable GraphCodeBERT encodes them inside the training loop. Cached to `.npz` for
resume. **Gate**: assert >= `MIN_UNIT_COVERAGE` of contracts yield >= 1 unit.
""")
...
if TOK_NPZ.exists():
    z = np.load(TOK_NPZ)
    TID, TAM, UM, UT = z["tid"], z["tam"], z["um"], z["ut"]
    log(f"Loaded cached tokenised units {TID.shape} {TID.dtype} from {TOK_NPZ.name}")
else:
    TID = np.zeros((N, MAX_UNITS, UNIT_TOKENS), dtype=np.int32)
    TAM = np.zeros((N, MAX_UNITS, UNIT_TOKENS), dtype=np.bool_)
    ...
    for row in range(N):
        ...
        enc = gcb_tokenizer(texts, padding="max_length", truncation=True,
                            max_length=UNIT_TOKENS, return_tensors="np")
        ids, am = enc["input_ids"], enc["attention_mask"]
        for slot in range(min(len(units), MAX_UNITS)):
            TID[row, slot] = ids[slot]
            TAM[row, slot] = am[slot].astype(bool)
            UM[row, slot]  = True
            UT[row, slot]  = 1 if units[slot][2] else 0
            tok_lens.append(int(am[slot].sum()))
        if row > 0 and row % 4000 == 0:
            log(f"  tokenised {row}/{N} contracts | elapsed {time.time()-t_start:.0f}s")

    np.savez_compressed(TOK_NPZ, tid=TID, tam=TAM, um=UM, ut=UT)
    log(f"Tokenisation cache -> {TOK_NPZ.name} | {time.time()-t_start:.0f}s total")
...
`TID` / `TAM` are `(N, MAX_UNITS, UNIT_TOKENS)` token ids / attention masks,
`UM` is `(N, MAX_UNITS)` bool ( True = real unit ), `UT` is `(N, MAX_UNITS)` int
( 0 = header, 1 = function ).
...
code("""TID_tr, TAM_tr, UM_tr, UT_tr, Y_tr = TID[idx_tr], TAM[idx_tr], UM[idx_tr], UT[idx_tr], Y[idx_tr]
TID_v,  TAM_v,  UM_v,  UT_v,  Y_v  = TID[idx_v],  TAM[idx_v],  UM[idx_v],  UT[idx_v],  Y[idx_v]
TID_te, TAM_te, UM_te, UT_te, Y_te = TID[idx_te], TAM[idx_te], UM[idx_te], UT[idx_te], Y[idx_te]
log(f"Token tensors | TID_tr {TID_tr.shape} {TID_tr.dtype} | mean real-units (train) = {UM_tr.sum(1).mean():.1f}")


class UnitDS(Dataset):
    def __init__(self, TID, TAM, UM, UT, Y, augment=False):
        self.TID, self.TAM, self.UM, self.UT, self.Y = TID, TAM, UM, UT, Y
    ...
    def __getitem__(self, i):
        ...
        if self.augment:
            ...
            if len(real) > 1:
                ...
                if not um.any():
                    um[real[0]] = True
        return (torch.from_numpy(self.TID[i].astype(np.int64)),
                torch.from_numpy(self.TAM[i]),
                torch.from_numpy(self.UT[i].astype(np.int64)),
                torch.from_numpy(um),
                torch.from_numpy(self.Y[i]))


ds_tr = UnitDS(TID_tr, TAM_tr, UM_tr, UT_tr, Y_tr, augment=True)
ds_v  = UnitDS(TID_v,  TAM_v,  UM_v,  UT_v,  Y_v,  augment=False)
ds_te = UnitDS(TID_te, TAM_te, UM_te, UT_te, Y_te, augment=False)
log(f"Datasets ready: train={len(ds_tr)} val={len(ds_v)} test={len(ds_te)}")
log(f"  aug: unit_drop={AUG_UNIT_DROP_PROB}")
""")
...
# ---------------------------------------------------------------------------
md("""## 11b — End-to-end model: GraphCodeBERT (top-K trainable) + aggregator

`GCBFTModel` encodes each contract's units with the partially-unfrozen
GraphCodeBERT, mean-pools each unit, then feeds the set-Transformer aggregator.
Only the **real** units go through the encoder (padding slots are skipped, which
avoids all-masked-sequence NaNs and saves compute).
""")

code("""class GCBFTModel(nn.Module):
    def __init__(self, encoder, aggregator):
        super().__init__()
        self.gcb = encoder
        self.agg = aggregator

    def forward(self, tid, tam, utype, umask):
        B, U, L = tid.shape
        flat_um  = umask.reshape(B * U)
        flat_ids = tid.reshape(B * U, L)
        flat_am  = tam.reshape(B * U, L).long()
        H = self.gcb.config.hidden_size
        emb = torch.zeros(B * U, H, device=tid.device, dtype=torch.float32)
        real = flat_um.nonzero(as_tuple=True)[0]
        if real.numel() > 0:
            out = self.gcb(input_ids=flat_ids[real], attention_mask=flat_am[real]).last_hidden_state
            mf = flat_am[real].unsqueeze(-1).float()
            pooled = (out.float() * mf).sum(1) / mf.sum(1).clamp(min=1.0)
            emb[real] = pooled
        emb = emb.view(B, U, H)
        return self.agg(emb, umask, utype)
""")
...
def build_ft_optimizer(model, enc_lr=ENC_LR, agg_lr=LR, weight_decay=WEIGHT_DECAY):
    g = {"enc_decay": [], "enc_nd": [], "agg_decay": [], "agg_nd": []}
    for n, p in model.named_parameters():
        if not p.requires_grad:
            continue
        nd = p.ndim <= 1 or n.endswith(".bias") or "norm" in n.lower() or "emb" in n.lower()
        prefix = "enc_" if n.startswith("gcb.") else "agg_"
        g[prefix + ("nd" if nd else "decay")].append(p)
    spec = [
        {"params": g["enc_decay"], "lr": enc_lr, "weight_decay": weight_decay},
        {"params": g["enc_nd"],    "lr": enc_lr, "weight_decay": 0.0},
        {"params": g["agg_decay"], "lr": agg_lr, "weight_decay": weight_decay},
        {"params": g["agg_nd"],    "lr": agg_lr, "weight_decay": 0.0},
    ]
    spec = [grp for grp in spec if len(grp["params"]) > 0]
    n_enc = sum(p.numel() for p in g["enc_decay"] + g["enc_nd"])
    n_agg = sum(p.numel() for p in g["agg_decay"] + g["agg_nd"])
    log(f"FT optimizer: encoder {n_enc/1e6:.2f}M @ lr={enc_lr:.0e} | aggregator {n_agg/1e6:.2f}M @ lr={agg_lr:.0e}")
    return torch.optim.AdamW(spec, betas=(0.9, 0.999), eps=1e-8)
...
def predict(model, loader):
    ...
    for tid, tam, ut, um, Yb in loader:
        tid = tid.to(device, non_blocking=True)
        tam = tam.to(device, non_blocking=True)
        ut  = ut.to(device, non_blocking=True)
        um  = um.to(device, non_blocking=True)
        with autocast():
            main_logits, _ = model(tid, tam, ut, um)
...
ds_tr_eval = UnitDS(TID_tr, TAM_tr, UM_tr, UT_tr, Y_tr, augment=False)
# FT makes a full-train pass expensive (the encoder runs), so the overfit probe uses a fixed subsample.
_probe = np.random.RandomState(0).choice(len(ds_tr_eval), min(2000, len(ds_tr_eval)), replace=False)
train_eval_loader = DataLoader(torch.utils.data.Subset(ds_tr_eval, _probe.tolist()),
                               batch_size=BATCH_SIZE, shuffle=False,
                               num_workers=NUM_WORKERS, pin_memory=PIN_MEMORY)
log(f"train overfit-probe: {len(_probe)} contracts (subsample)")
...
code("""agg   = UnitAggModel().to(device)
model = GCBFTModel(gcb, agg).to(device)
# single-GPU FT (DataParallel over a fine-tuned HF encoder is finicky); the wall-clock guard protects the budget.
optimizer  = build_ft_optimizer(model, enc_lr=ENC_LR, agg_lr=LR, weight_decay=WEIGHT_DECAY)
...
def _run_epoch(epoch):
    ...
    for step, (tid, tam, ut, um, Yb) in enumerate(train_loader, 1):
        tid = tid.to(device, non_blocking=True)
        tam = tam.to(device, non_blocking=True)
        ut  = ut.to(device,  non_blocking=True)
        um  = um.to(device,  non_blocking=True)
        ...
        with autocast():
            main_logits, aux_logits = model(tid, tam, ut, um)
        ...
        if time.time() - TRAIN_START > TIME_BUDGET_SECS:
            log(f"  [time budget] {TIME_BUDGET_SECS/3600:.1f}h reached at step {step}/{n_steps} — ending epoch early")
            break
...
try:
    for epoch in range(start_epoch, EPOCHS + 1):
        if time.time() - TRAIN_START > TIME_BUDGET_SECS:
            log(f"Time budget {TIME_BUDGET_SECS/3600:.1f}h reached — stopping before epoch {epoch}.")
            break
        epoch_save_done = False
...
if not BEST_PATH.exists():
    ...
    eval_model = GCBFTModel(gcb, UnitAggModel()).to(device)
...
def infer_contract(src_text):
    ...
    tid = torch.zeros(1, MAX_UNITS, UNIT_TOKENS, dtype=torch.long, device=device)
    tam = torch.zeros(1, MAX_UNITS, UNIT_TOKENS, dtype=torch.long, device=device)
    ut  = torch.zeros(1, MAX_UNITS, dtype=torch.long, device=device)
    um  = torch.zeros(1, MAX_UNITS, dtype=torch.bool, device=device)
    tid[0, :n] = enc["input_ids"][:n].to(device)
    tam[0, :n] = enc["attention_mask"][:n].to(device)
    ut[0, :n]  = torch.tensor(types[:n], device=device)
    um[0, :n]  = True

    with autocast():
        main_logits, _ = infer_model(tid, tam, ut, um)
...
2. Frozen GraphCodeBERT mean-pools each unit ( <=256 tokens ) into a 768-d
   vector — cached once to `dive_synth8_units.npz`.
...
- `cache/dive_synth8_units.npz` — frozen unit embeddings ( reused on resume ).
- `best_model.pt` — EMA weights + calibrators + frozen thresholds.
- `last_state.pt` — full resumable state ( `RESUME_FROM` ).
- `metrics.json`, `per_class.csv`, `history.csv`, `dive_synth8_train.log`.
...
out = Path(__file__).parent / "dive-synthesized-8.ipynb"
